...
const SearchProductDisplay = () => {
    ...
    if (min_price.length || max_price.length) {
      ...
      const filteredProductsData = allProducts.filter((product) =>
        product?.name
          .toString()
          .toLowerCase()
          .includes(searchProductName.toString().toLowerCase())
      );

      // filter products with these conditions
      const filtered = filteredProductsData.filter((product) => {
  ...
  //brand filtering form allProduct state and else ta filterdPrice mani price filter krer pore
  ...
  const uniqueMarchent = allProducts?.reduce((finalArray, current) => {
