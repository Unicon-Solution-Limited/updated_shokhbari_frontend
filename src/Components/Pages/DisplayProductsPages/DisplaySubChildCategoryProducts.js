...
const DisplaySubChildCategoryProducts = () => {
  ...
  useEffect(() => {
    ...
    const fetchFilteredProducts = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/getDisplaySubChildCategorisProduct?subChildCategories=${childCatvalue}`
        );
        const data = await response.json();
...
export default DisplaySubChildCategoryProducts;
